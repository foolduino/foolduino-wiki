...
src_trim=/tmp/mikewiki_src_trim
rm -f ${src_trim} ${src_trim}.swp
cp ${src} ${src_trim}
#gvim -u NONE -c ':%s/\([^$]\)\n\([^\x00-\x7f$]\)/\1\2/g | wq' ${src_trim}
#ssed -i ':a;N;$!ba;s/\([^$]\)\n\([^\x0-\x7f]\)/\1\2/g' /tmp/mikewiki_src_trim
...
PANDOC_FLAG+=" --include-after-body pageframe/head_statistic.html" # 百度统计
PANDOC_FLAG+=" -N"
...
sed -n -e "s/^<!---date:\(.\+\)-->/<p>date: \1<\/p>/p" $src >> temp_title.html # 生成<body>中的标题标签<h1>
cp utils/pandoctpl.html temp_pandoctpl.html
# WIKI_TOPDIR替换为相对地址 {{{
relative_dir=""
dst_dir=`readlink -f $src`
...
pandoc ${PANDOC_FLAG} --from=markdown --to=html ${src_trim} -o $dst # 调用pandoc编译

# 后处理
sed -i -e 's/<a href="#TOC">\(.\+\)<\/a>/\1/g' $dst # 去掉pandoc产生的从标题向目录的链接
sed -i -e 's/\(<h. id="[^"]\+">\)<a href="[^"]\+">\(.\+\)<\/a>/\1\2/g' $dst # 去掉pandoc产生的从标题向目录的链接
